...
layui.use(['table', 'jquery'], function () {
    ...
    table.render({
        elem: '#test',
        url: '/sysadmin/admin/usageManagement/getAdminUsageManagementData',
        cellMinWidth: 80,
        defaultToolbar: [],
        page: true,
        limits: [10],
        cols: [[{field: 'uId', fixed: 'left', width: 110, title: '用户编号', sort: true}, {
            ...
            field: 'uSysCount', width: 130, title: '今日剩余量', sort: true
        ...
        if (obj.event === 'edit') {
            const index = layer.open({
                ...
                content: '<form class="layui-form" lay-filter="val-filter" style="padding: 10px">\n'
                    ...
                    + '            <div class="layui-input-inline">\n'
                    + '                <input type="text" id="uTotalUse" name="uTotalUse" lay-verify="required|number"  readonly class="layui-input">\n'
                    + '            </div>\n'
                    + '        </div>\n'
                    + '    </div>\n'
                    + '    <div class="layui-form-item">\n'
                    + '        <div class="layui-inline">\n'
                    + '            <label class="layui-form-label">今日剩余量</label>\n'
                    + '            <div class="layui-input-inline">\n'
                ...
                success: function () {
                    ...
                    form.on('submit(edit-uCount)', function () {
                        $.ajax({
                            type: "post",
                            url: "/sysadmin/admin/usageManagement/editAdminUsageManagementData",
                            dataType: "json",
                            data: {
                                uSysCount: $("#uSysCount").val(),
                                uid: data.uId,
                            },
